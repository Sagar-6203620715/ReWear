...
router.post("/", protect, async (req, res) => {
  try {
    const { recipientItemId, recipientUserId, points } = req.body;
    
    // Check if user has an item to offer
    const userItems = await Item.find({ user: req.user.id, isActive: true });
    if (userItems.length === 0) {
      return res.status(400).json({ message: "You need to list an item first to make a swap request" });
    }

    // Get the first available item from the user
    const initiatorItem = userItems[0];

    // Check if recipient item exists and is available
    const recipientItem = await Item.findById(recipientItemId);
    if (!recipientItem || !recipientItem.isActive) {
...
router.put("/:id/complete", protect, async (req, res) => {
  try {
    ...
    await Item.findByIdAndUpdate(swap.initiatorItem, { isActive: false });
    await Item.findByIdAndUpdate(swap.recipientItem, { isActive: false });
